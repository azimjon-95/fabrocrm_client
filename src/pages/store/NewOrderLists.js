...
const NewOrderList = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "download",
            render: (record) => (
                <Button type="primary" onClick={() => handleExport(record)} style={{ background: "#0A3D3A" }}>
    ...
    const handleExport = (record) => {
        ...
        wsData.push([
            ...
            `${dayjs(order.createdAt).date()}-${uzMonths[dayjs(order.createdAt).month()]} / ${dayjs(order.createdAt).format("HH:mm")}`
        ]);
        ...
        const colWidths = wsData[0].map((_, i) =>
            Math.max(...wsData.map(row => (row[i] ? row[i].toString().length : 10))) + 2
        );
