...
const ViewOrder = () => {
  ...
  const newOrders = useMemo(
    () => orders?.innerData?.filter((order) => order.isType === true) || [],
    [orders]
  );
      ...
      <div className="order-cards">
        {filteredOrders.map((order) => (
          <Card
            ...
            hoverable
            cover={
              <Image src={order.image} alt="Mebel" className="Image-orders" />
            }
            actions={[
              <Button
                icon={<EditOutlined />}
                onClick={() => navigate(`/orders/materials/${order._id}`)}
              />,
              <Button
                icon={<EyeOutlined />}
                onClick={() => showCustomerInfo(order?.customer)}
              />,
            ]}
          >
            <Card.Meta
              title={order.name}
              description={`Tulov turi: ${order.paymentType}`}
            />
            ...
            {Object.entries(selectedCustomer)
              .filter(
                ([key, value]) =>
                  ...
                  value !== "" &&
                  key !== "_id"
              ) // _id, undefined, null va bo'sh qiymatlarni olib tashlash
              .map(([key, value], index) => {
                const labels = {
                  type: "Turi",
                  fullName: "F.I.O.",
