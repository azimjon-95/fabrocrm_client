...
const CtrlWorkers = () => {
    ...
    const filteredAdmins = workers.filter(worker => adminRoles.includes(worker.role));

    // Ishchilar (admin roli yo‘qlar)
    const filteredWorkers = workers.filter(worker => !adminRoles.includes(worker.role));
                ...
                <Tabs.TabPane tab="Ishchilar" key="all">
                    <div className="worker-list">
                        {filteredWorkers.map((worker) => {
                            const { formattedDate, adjustedAge, birthdayMessage } = formatBirthDate(worker.dayOfBirth);
                            return (
                                <div className="worker-card" key={worker._id}>
                                    {worker.img ?
                                        ...
                                        />
                                        : <div style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: "35px", width: '80px', height: '80px', borderRadius: '10px' }}><UserOutlined /></div>}
                                    <div className="worker-card-main">
                                        <div className="worker-card-header">
                                            <div style={{ marginTop: "15px" }} className="worker-details">
                                                <p>{worker.workerType || "Noma’lum rol"}</p>
                                                <span className="worker-name">{worker.firstName} {worker.lastName}</span>
                                                <span className="worker-phone">{formatPhoneNumber(worker.phone)}</span>
                                                <span className="worker-address">{worker.address}</span>
                                                <div className="worker-birthdate">
                                                    {formattedDate} ({adjustedAge} yosh)
                                                </div>
                                                {birthdayMessage && <div className="birthday-message">{birthdayMessage}</div>}
                                            </div>
                                        </div>
                                        <div className="worker-id-header">
                                            <button onClick={() => toggleVisibleId(worker._id)} className="toggle-id-btn">
                                                {visibleId === worker._id ? <EyeInvisibleOutlined /> : <EyeOutlined />}
                                            </button>
                                            <span>{visibleId === worker._id ? worker.idNumber : "*********"}</span>
                                        </div>
                                    </div>
                                </div>
                            );
                                    ...
                                    <div className="worker-card-main">
                                        <div className="worker-card-header">
                                            <div className="worker-details">
                                                ...
                                                <span className="worker-name">{worker.lastName} {worker.firstName} {worker.middleName}</span>
                                                <span className="worker-phone">{formatPhoneNumber(worker.phone)}</span>
                                                <span className="worker-address">{worker.address}</span>
                                                <div className="worker-birthdate">
                                                    {formattedDate} ({adjustedAge} yosh)
