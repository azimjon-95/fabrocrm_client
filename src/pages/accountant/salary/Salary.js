...
import { Table, Spin, Alert, DatePicker, message, Button, Form, Radio, InputNumber, Tooltip } from "antd";
...
const Salary = () => {
    ...
    const [formState, setFormState] = useState({ paymentType: "salary", amount: null });
    ...
    const handleSubmit = useCallback((remainingSalary, paymentType, amount, record) => {
        if (amount > 0 && amount <= remainingSalary) {
            message.success("Ma'lumot yuborildi:", { paymentType, amount, workerId: record.workerId });
            console.log("Ma'lumot yuborildi:", { paymentType, amount, workerId: record.workerId });
            ...
            message.warning("Salom, avans yoki ish haqidan katta summa kiritilmadi!");
        }
    }, [createSalary]);
    ...
    const SalaryPaymentForm = ({ remainingSalary, record }) => {
        ...
        return (
            <Form style={{ display: "flex", alignItems: "center", gap: "10px" }} >
                <Form.Item>
                    <Radio.Group onChange={(e) => handleChange("paymentType", e.target.value)} value={paymentType}>
                ...
                <Button
                    type="primary"
                    htmlType="submit"
                    onClick={() => handleSubmit(remainingSalary, paymentType, amount, record)}
    ...
    const columns = [
        { title: "Ism Familya", dataIndex: "workerName", key: "workerName", fixed: 'left' },
        {
        ...
        { title: "Jami maosh", dataIndex: "totalSalary", key: "totalSalary", render: (text) => `${text.toLocaleString()} so'm` },
        {
            ...
            render: (_, record) => {
                ...
                return (
                    <>
                        <div><strong>Avans:</strong> {(record.advance || 0).toLocaleString()} so‘m</div>
                        <div><strong>Qoldiq:</strong> {remainingSalary.toLocaleString()} so‘m</div>
                    </>
                );
            },
        },
        {
            title: `Maosh Va Avans Berish`, key: "payment", fixed: 'right', render: (_, record) => {
                const remainingSalary = record.totalSalary - (record.advance || 0); // Avans ayirilgan jami maosh
                return <SalaryPaymentForm remainingSalary={remainingSalary} record={record} />;
            },
        },
    ];
